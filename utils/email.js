...
const transporter = nodemailer.createTransport({
  service: "Gmail",
  auth: {
    user: 'fruitco25@gmail.com',
    pass: 'crcj dwce hygk wegc',
  },
});
...
// const a_title = `Fruit Co Order Placed. Order Id : ${orderId}`
...
background-color: #4CAF50; color: white; padding: 10px 0; border-radius: 8px 8px 0 0; } .header .checkmark { display: inline-block; width: 60px;
 ...
 color: #888; background-color: #f0f0f0; border-radius: 0 0 8px 8px; } .button-container { text-align: center; margin: 20px 0;
     ...
     <div class="header"> <div class="checkmark">✔️</div> <h1>Order Successfully Placed</h1> </div> <div class="order-details"> 
     <b>Thank you for your order at Fruit Co. Here are the details:</b> 
     <table> <tr> <th>Detail</th> <th>Information</th> 
     </tr><tr> <td>OrderId</td> <td>${order._id}</td> </tr> <tr> <td>Name</td> <td>${order.name}</td> </tr> <tr> <td>Address</td> <td>${order.address}</td> </tr> 
     <tr> <td>Phone</td> <td>${order.phone}</td> </tr> <tr> <td>Plan</td> <td>${order.plan}</td> </tr> 
     <tr> <td>Delivery Date</td> <td>${order.deliveryDate.toLocaleString()}</td> 
     </tr> </table> <div class="button-container"> <a href="${orderStatusUrl}">Check Order Status</a> 
     </div> </div> <div class="footer"> <p>Thank you for choosing us!</p> <p>From the Team, Fruit Co</p> 
     <p>&copy; 2024 Fruit Bowl. All rights reserved.</p> </div> </div> </body> </html>`
